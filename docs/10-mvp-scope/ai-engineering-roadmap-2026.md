...
`fixtures/offline_mvp/cases.json`. Die Docker-Grenze installiert diese Datei
nach der nicht-editable Paketinstallation am bestehenden generischen
`DEFAULT_FIXTURE_PATH`; der Anwendungskern enthaelt keine Docker- oder
Deployment-Konfiguration. Containerabhaengigkeiten werden vollstaendig ueber
`requirements-runtime.lock` mit exakten Pins und Hashes installiert; das
...
Paketpfad und Fixture bleiben root-owned und fuer UID `10001` nicht
beschreibbar. Ein separater CI-Docker-Smoke-Job prueft Allowlist,
Nicht-Schreibbarkeit, Docker-Health sowie `/health`, `/version` und
`POST /ai/draft`. Standard-pytest bleibt docker- und netzwerkfrei.
...
- ein kurzlebiger Amazon-ECS-Express-Mode-Dienst in `eu-central-1`
- `GET /health` und ein synthetischer `POST /ai/draft` mit
  `FakeModelProvider`
- strukturierte CloudWatch-Logs und EMF-Nachweis
...
- Umfang: deny-by-default Context-Allowlist, ausschliesslich
  `fixtures/offline_mvp/cases.json`, Installation dieser Datei am bestehenden
  generischen `DEFAULT_FIXTURE_PATH` durch die Docker-Grenze, gepinntes
  Python-/Debian-Basisimage inkl. Digest, `requirements-runtime.lock` mit
  exakten Pins und Hashes, nicht-editable lokale Paketinstallation mit
  `--no-deps` und `--no-build-isolation`, root-owned nicht beschreibbarer
  Paket- und Fixture-Pfad, Non-Root-User, dynamischer Docker-Health-Smoke
  sowie schlanke statische Offline-Tests. Keine Docker-Konfiguration im
  Anwendungskern. `FakeModelProvider` bleibt sicherer Standard;
